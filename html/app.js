...
window.onload = function () {
  var eventCallback = {
    ...
    updateVehicle: function (data) {
      ...
      if (data.status == true) {
        ...
        var previousGear =
          document.querySelector("#vehicle-gear span").innerHTML;
  ...
  window.addEventListener("message", (event) => {
    ...
    if (data.type === "display:incar") {
      if (data.incar == true) {
        var status = document.getElementById("status");
        status.classList.add("isInCar");
        status.classList.remove("isNotCar");
      } else {
        var status = document.getElementById("status");
        status.classList.remove("isInCar");
        status.classList.add("isNotCar");
      }
    } else if (data.type === "updatePlayerJobs") {
      var jobs = data.jobs;
      document.getElementById("onlineplayers").textContent = jobs.onlineplayers;
      document.getElementById("ems").textContent = jobs.ems;
      document.getElementById("police").textContent = jobs.police;
      document.getElementById("taxi").textContent = jobs.taxi;
      document.getElementById("mechanic").textContent = jobs.mechanic;
    } else if (data.type === "ShowZonaSegura") {
      if (data.displayZone == true) {
        document.querySelector("#zonasegura").style.display = "block";
      } else {
        document.querySelector("#zonasegura").style.display = "none";
      }
    } else {
      eventCallback[data.action](data);
    }
  });
};
