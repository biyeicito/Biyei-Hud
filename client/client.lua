...
RegisterNetEvent('biyei_hud:ocultar', function()
    SendNUIMessage({
        action = 'toggleUi',
        value = false
    })
...
RegisterNetEvent('biyei_hud:mostrar', function()
    SendNUIMessage({
        action = 'toggleUi',
        value = true
    })
    ...
    while true do
        ...
        if IsPauseMenuActive() then -- ESC Key
            if not isPauseMenu then
                ...
                TriggerEvent('biyei_hud:ocultar')
            end
        else
            if isPauseMenu then
                isPauseMenu = not isPauseMenu
                TriggerEvent('biyei_hud:mostrar')
            end
...
RegisterKeyMapping('limitarvelocidad', 'Limitar Velocidad', 'keyboard', Config.vehicle.keys.cruiser)

RegisterCommand('limitarvelocidad', function()
    local player = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleClass = GetVehicleClass(vehicle)
...
RegisterKeyMapping('cinturon', 'Ponerse/Quitarse Cinturon', 'keyboard', Config.vehicle.keys.seatbelt)
...
RegisterKeyMapping('guiaizquierda', 'Guia a la izquierda', 'keyboard', Config.vehicle.keys.signalLeft)
RegisterCommand('guiaizquierda', function()
    local player = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleClass = GetVehicleClass(vehicle)
...
RegisterKeyMapping('guiaderecha', 'Guia a la Derecha', 'keyboard', Config.vehicle.keys.signalRight)
RegisterCommand('guiaderecha', function()
    local player = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleClass = GetVehicleClass(vehicle)
...
RegisterKeyMapping('lucesdeestacionamiento', 'Luces Estacionamiento', 'keyboard', Config.vehicle.keys.signalBoth)
RegisterCommand('lucesdeestacionamiento', function()
    local player = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(player, false)
    local vehicleClass = GetVehicleClass(vehicle)
...
function UpdatePlayerTable(connectedPlayers)
    ...
    local ems, police, swat, taxi, mechanic, armero, idplayer, onlineplayers = 0, 0, 0, 0, 0, 0, 0, 0

    for k, v in pairs(connectedPlayers) do

        if v.job == Config.AmbulanceJob then
            ems = ems + 1
        elseif v.job == Config.PoliceJob then
            police = police + 1
        elseif v.job == Config.TaxiJob then
            taxi = taxi + 1
        elseif v.job == Config.MechanicJob then
            mechanic = mechanic + 1
        end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for i = 1, #Config.SecureZones, 1 do
            dist = Vdist(Config.SecureZones[i].x, Config.SecureZones[i].y, Config.SecureZones[i].z, x, y, z)
...
--------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------
local vehicleSpeedSource = Config.MaxSpeedSource
local Spawned = false
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if Spawned then
            ...
            local showedNotiVeh = false

            local player = PlayerPedId()
            local x, y, z = table.unpack(GetEntityCoords(player, true))
            local dist = Vdist(Config.SecureZones[closestZone].x, Config.SecureZones[closestZone].y, Config.SecureZones[closestZone].z, x, y, z)
            local veh = GetVehiclePedIsIn(player, false)
            if dist <= Config.RadioSecureZone then ------------------------------------------------------------------------------ Here you can change the RADIUS of the Safe Zone. Remember, whatever you put here will DOUBLE because 

                sleep = 1

                if not notifIn then
                    if IsPedInAnyVehicle(player, false) and showedNotiVeh == false then
                        if ESX.PlayerData.job.name == Config.PoliceJob or ESX.PlayerData.job.name == Config.AmbulanceJob then
                            ESX.ShowNotification(
                                'Tu velocidad no limitara en Zona Segura porque estas en una facción legal')
                        else
                            ESX.ShowNotification('Tu velocidad se ha limitado a 50km/h porque estas en una Zona Segura')
                            SetEntityMaxSpeed(veh, 14.0)
                        end
                        showedNotiVeh = true
                    end

                    -- ESX.ShowNotification('Estas en Zona Segura')
                ...
                if not notifOut then
                    ...
                    -- ESX.ShowNotification('Cuidado, ya no estas en Zona Segura')
            ...
            if notifIn then
                ...
                if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' or ESX.PlayerData.job.name == 'ambulance' then
                    ...
                    DisableControlAction(2, 45, true) -- disable weapon wheel (Tab)
                    DisableControlAction(2, 140, true) -- disable weapon wheel (Tab)
